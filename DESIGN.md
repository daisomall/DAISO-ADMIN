...
> **토큰 모델**: 색/간격/radius/effect = `var(--color-*)`(SCSS 가 `:root` 로 출력), 타이포 = `@include typo-*` 믹스인.
...
> `<style scoped lang="scss">`. 블록 클래스 `da-{name}`, 토큰은 색/간격/radius `var(--color-*)` + 타이포 `@include typo-*`.
...
- [ ] 색·spacing·radius·effect 가 **`var(--color-*)`**, 타이포가 **`@include typo-*`** (raw hex/px 0건, 단 주석단 hairline 예외).
